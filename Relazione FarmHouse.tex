...
\begin{titlepage} % Suppresses displaying the page number on the title page and the subsequent page counts as page 1
	...
	\parbox[b]{0.75\textwidth}{ % Paragraph box for holding the title page text, adjust the width to move the title page left or right on the page
		...
		{\Huge\bfseries Relazione per il Progetto di Base Dati 2019}\\[2\baselineskip] % Title
		{\large\textit{FarmHouse 4.0}}\\[4\baselineskip] % Subtitle or further description
		{\Large\textsc{Guillaume Quint,\\ Francesco Bonciani}} % Author name, lower case for consistent small caps
		
		\vspace{0.5\textheight} % Whitespace between the title block and the publisher
		
		{\noindent Università di Pisa, Ingegneria Informatica\\\today}\\[\baselineskip] % Publisher and logo
	}
...
In accordo alle specifiche di progetto fornite, si è scelto di schematizzare la base dati in cinque aree tematiche:, \textit{Area Allevamento}, \textit{Area Healthcare}, \textit{Area Produzione}, \textit{Area Soggiorno}, \textit{Area Store}.
Queste sono state elaborate sfruttando una visione \textit{bottom-up} per quanto riguarda la descrizione caratteristica di ogni entità, mentre si è preferito una strategia \textit{top-down} nell'evidenziare le relazioni tra le varie aree.
Di fatto, è stato sfruttato il meglio dei due approcci, consentendo complessivamente l'utilizzo di una strategia mista.
\end{abstract}
\pagebreak
\tableofcontents
\pagebreak
...
\hline
Condizio\-na\-to\-re aria 		&  Dispositivo di condizionamento dell'aria all'interno dei locali
					& \begin{itemize}
						\setlength{\itemindent}{-1em}
						\vspace{-25pt}
						\setlength\itemsep{-0.25em}
...
\hline
Illuminazio\-ne 		&  Dispositivo di illuminazione artificiale dei locali
					& \begin{itemize}
						\setlength{\itemindent}{-1em}
						\vspace{-25pt}
						\setlength\itemsep{-0.25em}
...
\hline
divisione\ \ condizionatori 				&   
					& \begin{itemize}
						\setlength{\itemindent}{-1em}
						\vspace{-25pt}
						\setlength\itemsep{-0.25em}
						\item (1,N) con Locale: Ogni locale è dotato di uno o più condizionatori
						\item (1,1) con Condizionatore aria: un condizionatore è associato ad un solo locale
					\end{itemize}\\ 
...
divisione\ \ \ \ illuminazione 				&   
					& \begin{itemize}
						...
						\vspace{-25pt}
						\setlength\itemsep{-0.25em}
						\item (1,N) con Locale: Ogni locale è dotato di uno o più dispositivi di illuminazione
						\item (1,1) con Illuminazione: un dispositivo di illuminazione è associato ad un solo locale
					\end{itemize}\\ 
...
richiesta intervento 				&   
					& \begin{itemize}
						...
						\vspace{-25pt}
						\setlength\itemsep{-0.25em}
						\item (0,N) con Locale: alcuni locali possono richiedere più interventi di pulizia
...
possiede terapia 				&  
					& \begin{itemize}
						...
						\vspace{-25pt}
						\setlength\itemsep{-0.25em}
						\item (0,N) con Animale: alcuni animali possono possedere più terapie
...
Controllo Parametri Fase				 	&  Controlla, per ogni lotto, i parametri effettivi relativi ad ogni singola fase di produzione del formaggio 
					& \begin{itemize}
						...
						\vspace{-25pt}
						\setlength\itemsep{-0.25em}
						\item durataFaseEffettiva
						\item temperaturaLatteEffettiva
						\item tempoRiposoEffettivo
						\item codiceLotto
						\item codiceFase
					\end{itemize}
					&  Ricetta, Lotto  \\ 
...
Fasi				 	&  Singola istruzione con informazioni da seguire per la produzione ottimale del formaggio da produrre 
					& \begin{itemize}
						...
						\item nome
						\item durata
						\item durataFaseIdeale
						\item temperaturaLatteIdeale
						\item tempoRiposoIdeale
					\end{itemize}
					&  Ricetta, Lotto  \\ 
...
con 				&   
					& \begin{itemize}
						...
						\item (1,N) con Mungitrice: ogni mungitrice può essere impiegata per diverse mungiture
					\end{itemize}\\ 
...
stoccaggio cantine 				&   
					& \begin{itemize}
						...
						\vspace{-25pt}
						\setlength\itemsep{-0.25em}
						\item (0,N) con Lotto: i lotti che richiedono stagionatura possono stoccare o propri formaggi in più cantine
...
effettua 				&   
					& \begin{itemize}
						...
						\vspace{-25pt}
						\setlength\itemsep{-0.25em}
						\item (0,N) con Cliente: alcuni clienti possono effettuare più pagamenti
...
\item Si mantiene la ridondanza di \textit{capienza max} di Locale seppur possa essere ricavata dalla specie ospitata e dalle dimensioni dello stesso
...
\hline
Condizionatore aria 				& \begin{center}
\vspace{-25pt}E
\end{center}
					& \begin{center}
					\vspace{-25pt}500\end{center}
					&  Ogni locale è dotato di un condizionatore dell'aria \\ 
...
\hline
Illuminazione 				& \begin{center}
\vspace{-25pt}E
\end{center}
					& \begin{center}
					\vspace{-25pt}500\end{center}
					&  Ogni locale è dotato di un dispositivo di illuminazione \\ 
...
\includegraphics[scale=2, width=1\textwidth]{operazioni/schemi_parziali/operazione-\thecounterAccessi.png} 
...
\includegraphics[scale=2, width=1.1\textwidth]{operazioni/schemi_parziali/operazione-\thecounterAccessi.png} 
\subsubsection*{Porzione della tavola dei volumi interessata:}
\begin{center}\setlength{\extrarowheight}{1.5pt}\begin{longtable}{|p{0.23\linewidth}|p{0.1\linewidth}|p{0.11\linewidth}|p{0.45\linewidth}|}
\hline \textbf{Nome}   & \begin{center}\vspace{-15pt}\textbf{E/R}\end{center} & \textbf{Numero Istanze} & \textbf{Motivazione}\\ 
\hline
...
\includegraphics[scale=2, width=0.8\textwidth]{operazioni/schemi_parziali/operazione-\thecounterAccessi.png} 
\subsubsection*{Porzione della tavola dei volumi interessata:}
\begin{center}\setlength{\extrarowheight}{1.5pt}\begin{longtable}{|p{0.23\linewidth}|p{0.1\linewidth}|p{0.11\linewidth}|p{0.45\linewidth}|}
\hline \textbf{Nome}   & \begin{center}\vspace{-15pt}\textbf{E/R}\end{center} & \textbf{Numero Istanze} & \textbf{Motivazione}\\ 
\hline
...
\item Agriturismo(\underline{\smash{Nome}}, indirizzo, indirizzoWeb, contatto)
...
\item Illuminazione(\underline{\smash{codice}}, consumoEnergetico, codLocale)
\item CondizionatoreAria(\underline{\smash{codice}}, consumoEnergetico, codLocale)
\item PuliziaLocale(\underline{\smash{codLocale}}, \underline{\smash{orarioRilevazione}}, \underline{\smash{dataRilevazione}}, stato, personale, concentrazioneMetano, concentrazioneAzoto, livelloSporcizia)
...
j\item Mangiatoia(\underline{\smash{codice}}, quantitàRestante,  codLocale)
...
\item ControlloParametriFase(\underline{\smash{durataFaseIdeale}}, \underline{\smash{temperaturaLatteIdeale}},\underline{\smash{tempoRiposoIdeale}},\underline{\smash{codLotto}}, \underline{\smash{codFase}})
\item Fasi(\underline{\smash{codiceFase}}, nome, durataFaseEffettiva, temperaturaLatteEffettiva, tempoRiposoEffettivo, durata)
...
\item Formaggio(\underline{\smash{nome}}, \underline{\smash{nomeAgriturismo}}, deperibilita,  codRicetta)
...
\begin{itemize}
   ...
   \item Esiste un vincolo di integrità referenziale tra l'attributo codiceLocale in locale e codLocale in Illuminazione
   \item Esiste un vincolo di integrità referenziale tra l'attributo codiceLocale in locale e codLocale in CondizionatoreAria
...
In questo capitolo viene affrontata la normalizzazione delle tabelle ricavate, al fine di raggiungere uno schema logico composto solamente da tabelle in Forma normale di Boyce-Codd.
\begin{enumerate}
...
\begin{itemize}
\vspace{-5pt}
\item codice $\Rightarrow$ sesso, specie, famiglia, idMadre, idPadre, dataDiNascita, altezza, peso, razza, posizione, orario, codiceGps, codLocale
\vspace{-5pt}
\end{itemize}
...
Agriturismo(\underline{\smash{Nome}}, indirizzo, indirizzoWeb, contatto)

\begin{itemize}
\vspace{-5pt}
\item Nome $\Rightarrow$ indirizzo, indirizzoWeb, contatto
\vspace{-5pt}
\end{itemize}
...
\item[] \textbf{Tabella Illuminazione:}

CompostiVolatili(\underline{\smash{codice}}, consumoEnergetico, codLocale)
\begin{itemize}
\vspace{-5pt}
\item codice $\Rightarrow$ consumoEnergetico, codLocale
\vspace{-5pt}
\end{itemize}
...
\item[] \textbf{Tabella CondizionatoreAria:}
...
PuliziaLocale(\underline{\smash{codLocale}}, \underline{\smash{orarioRilevazione}}, \underline{\smash{dataRilevazione}}, stato, personale, concentrazioneMetano, concentrazioneAzoto, livelloSporcizia)
\begin{itemize}
\vspace{-5pt}
\item codLocale, orarioRilevazione, dataRilevazione $\Rightarrow$ stato, personale, concentrazioneMetano, concentrazioneAzoto, livelloSporcizia
\vspace{-5pt}
\end{itemize}
...
ControlloParametriFase(\underline{\smash{durataFaseEffettiva}}, \underline{\smash{temperaturaLatteEffettiva}},\underline{\smash{tempoRiposoEffettivo}},\underline{\smash{codLotto}}, \underline{\smash{codFase}})
...
Fasi(\underline{\smash{codiceFase}}, nome, durataFaseIdeale, temperaturaLatteIdeale, tempoRiposoIdeale, durata)
\begin{itemize}
\vspace{-5pt}
\item codiceFase $\Rightarrow$ nome, durataFaseIdeale, temperaturaLatteIdeale, tempoRiposoIdeale, durata
\vspace{-5pt}
\end{itemize}
...
Formaggio(\underline{\smash{nome}}, \underline{\smash{nomeAgriturismo}}, deperibilita,  codRicetta)
\begin{itemize}
\vspace{-5pt}
\item nome, nomeAgriturismo $\Rightarrow$ deperibilità, codRicetta
\vspace{-5pt}
\end{itemize}
...
Sono state sviluppate quattro funzionalità di data analytic capaci di aggregare ed evidenziare dati salienti per il corretto ed efficiente uso del database per la gestione delle imprese legate alla catena di agriturismi \textit{FarmHouse 4.0}\\
\begin{description}
\item[AN\_salute\_gruppi: ]La funzione sfrutta le informazioni dei GPS per selezionare i capi di bestiame in gruppi di vicinanza (i gruppi contengono animali vicini entro un certo range e vengono formati in modo procedurale attraverso l'utilizzo di un cursore e scorrendo gli animali precedentemente ordinati per latitudine crescente attraverso un rank). Così facendo si ricavano gli indici di salute medi di ogni gruppo tramite gli indici di salute associati. La funzione permette di verificare fenomeni di emarginamento o vicinanza tra animali di costituzione fisica differente. Con sufficienti dati è possibile far emergere pattern ricorrenti, soprattutto in fase di pascolo, utili per prevenire il diffondersi di eventuali patologie, e per confrontare lo stato di salute di animali che pascolano in precise zone geografiche, in modo da poter eventualmente spostare le aree di pascolo nelle zone più frequentate così da garantire un minore stato di stress fisico agli animali..
\item[AN\_tracciabilità\_filiera: ]La funzione analizza i singoli prodotti venduti sullo store online costruendo una view in cui sono affiancati ad ognuno di essi un resoconto dei parametri di processo, la recensione media da parte dei clienti, oltre che l'informazione se il il prodotto è stato reso oppure no. Da ciò attraverso una tabella pivot si evidenziano quali caratteristiche del processo produttivo hanno un maggiore impatto sulla qualità percepita del prodotto. La funzione esprime inoltre la probabilità con cui il prodotto verrà reso in base a ciascuno dei parametri di produzione.
\item[AN\_controllo\_vendite: ]La funzione effettua un rank delle tipologie di formaggio vendute sullo store in funzione delle vendite e degli incassi generati, tenendo conto anche degli eventuali resi. Inoltre affianca ad ognuno di essi la valutazione media da parte dei clienti che hanno recensito il prodotto, così da evidenziare i formaggi di tendenza, o al contrario, quelli ben apprezzati seppur poco conosciuti. Questo permette, tra l'altro, di proporre con maggior fiducia i prodotti che hanno generato minor guadagno.
\item[AN\_controllo\_qualità\_del\_processo: ]La funzione stila un rank dei singoli lotti della produzione in base all'attinenza di questi ultimi con i parametri ideali di produzione e all'omogeneità delle componenti utilizzate, dalla composizione chimico-fisica del latte agli sbalzi di salute degli animali, in funzione anche dell'apporto alimentare dei pasti somministrati. Questi dati vengono poi visualizzati attraverso una tabella pivot per garantire una visione più immediata dell'andamento della qualità del prodotto conseguentemente ai vari parametri.
\end{description}
\end{document}
